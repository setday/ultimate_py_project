...
double GasContainer2d::calculateFlow(int x, int y, bool isHorizontal) const {
  if (isHorizontal)
    return _velocityX[y][x + 1];
  return _velocityY[y + 1][x];
}
...
void GasContainer2d::calculateFlows(double dt) {
  for (int row = 0; row < _height; ++row) {
    for (int column = 0; column < _width; ++column) {
      ...
      if (row < _height - 1) calculateAndSaveFlow(cell, _storage[row + 1][column], column, row, false);
      if (column < _width - 1) calculateAndSaveFlow(cell, _storage[row][column + 1], column, row, true);
...
void GasContainer2d::applyFlows(double dt) {
  for (int row = 0; row < _height; ++row) {
    for (int column = 0; column < _width; ++column) {
      ...
      if (row < _height - 1) {
        double targetFlow = calculateFlow(column, row, false);
        applyFlow(cell, _temporaryStorage[row + 1][column], targetFlow);
      }
      if (column < _width - 1) {
        double targetFlow = calculateFlow(column, row, true);
...
double GasContainer2d::getVelocityDiff(int x, int y, bool isHorizontal) {
  if (isHorizontal)
    return _velocityX[y][x + 1] - _velocityX[y][x];
  return _velocityY[y + 1][x] - _velocityY[y][x];
}

void GasContainer2d::updateCellsVelocity(int x, int y, bool isHorizontal) {
  if (isHorizontal)
    _velocityX[y][x + 1] = (_storage[y][x + 1].targetFlow - _storage[y][x].targetFlow) / cellSize;
  else
    _velocityY[y + 1][x] = (_storage[y + 1][x].targetFlow - _storage[y][x].targetFlow) / cellSize;
}
...
void GasContainer2d::updateVelocities(double dt) {
  for (int row = 0; row < _height; ++row) {
    for (int column = 0; column < _width; ++column) {
      if (row < _height - 1)
        updateCellsVelocity(column, row, false);
      if (column < _width - 1)
        updateCellsVelocity(column, row, true);

      _storage[row][column].particlesFlow =
              _velocityX[row][column] +
              _velocityX[row][column + 1] +
              _velocityY[row][column] +
              _velocityY[row + 1][column];
    }
  }
}
